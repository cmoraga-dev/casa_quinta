...
include_once __DIR__.'/User.php';
    ...
    class Booking extends DB{
        ...
        {
            $query = $this->connect()->query("SELECT b.id as 'id', datatime,confirmHour, CONCAT(u.first_name,' ' ,u.last_name) as 'full_name', ifnull(box_id,0) as 'box_id', active
                                                FROM `bookings` b INNER JOIN `users` u ON b.user_id = u.id 
                                                WHERE DATE(datatime) = DATE(NOW()) or DATE(confirmHour) = DATE(NOW())
        ...
        function createBooking($user_id, $dataTime){
            ...
            $query = $this->connect()->prepare("INSERT INTO bookings (`user_id`, `datatime`, `active`) 
                    VALUES((SELECT users.id FROM users WHERE users.identification_number =  '$user_id'), '$dataTime', 1)");
        ...
        function updateConfirmBooking($rut){
            ...
            if($responseRutByUser != 0 ){
                ...
                $query = $this->connect()->query("UPDATE bookings SET confirmed = 1, active = 1, confirmHour = NOW() WHERE `user_id` = $id_usuario AND ifnull(confirmed,0) = 0");
                return $query;
            }
